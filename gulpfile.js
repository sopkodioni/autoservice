...
gulp.task('jsLibs', function(){
    return gulp.src([
        'app/js/libs/simplebar.min.js' // SimpleBar
    ])
...
// Объеденение всех JS файлов в один файл (main.min.js), затем минификация (uglify), выгрузка в папку 'scripts' и обновление страницы после изменения
...
gulp.task('watch', function(){
    ...
    gulp.watch('app/js/**/*.js', gulp.parallel('script', 'js', 'jsLibs'));
});

    // дефолтный таск, который запускает поочередно плагины и создает live server
gulp.task('default', gulp.parallel('css', 'jsLibs', 'js', 'scss', 'browser-sync', 'watch'));
